                   ...
                   int ttyfd)
{
     ...
     * some mount.
     */
    if (virAsprintf(&rootfs_mount, "/%s/%s.rootfs_mount", LXC_STATE_DIR, def->name) < 0) {
        virReportError(VIR_ERR_INTERNAL_ERROR, "%s",
    ...
    virCommandAddArgList(cmd, "--pidfile", "pidfile", NULL);
    ...
    ignore_value(virCommandExec(cmd));

    ret = -1;

 cleanup:
    if (umount(rootfs_mount) < 0) {
        char ebuf[1024];
        virReportError(VIR_ERR_INTERNAL_ERROR,
                       _("Failed to unmount '%s': %s"),
                       rootfs_mount, virStrerror(errno, ebuf, sizeof(ebuf)));
    }

    if (rmdir(rootfs_mount) < 0) {
        virReportError(VIR_ERR_INTERNAL_ERROR,
                       _("Failed to delete %s"), rootfs_mount);
    }
