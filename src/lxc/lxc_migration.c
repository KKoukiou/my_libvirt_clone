...
lxcDoMigrateReceive(void *opaque)
{
    ...
    /* After dup system call the two descriptors do not
     * share file descriptor flags.
     * Thus, we have to set blocking flag again.
     * TODO: Is there a wrapper function for this? */
    flags = fcntl(recvfd, F_GETFL, 0);
    if (flags == -1)
        goto cleanup;
    /* Clear the non blocking flag. */
    flags &= ~O_NONBLOCK;
    if (fcntl(recvfd, F_SETFL, flags) == -1)
        goto cleanup;

    /* Store the files received in recvfd a tar file*/
    filefd = open("checkpoint.tar.gz",
    ...
    do {
        ...
        if (nbytes < 0) {
            ...
            if (safewrite(filefd, buffer, nbytes) != nbytes) {
                 ...
                 int sockfd)
{
    ...
    if ((ret = lxcCriuDump(driver, vm)) != 0) {
                        ...
                        const char *xmlin)
{
    ...
    if (xmlin) {
        ...
        if (!virDomainDefCheckABIStability(vm->def, tmpdef))
            goto endjob;
...
#else
char *
lxcDomainMigrationBegin(virConnectPtr conn ATTRIBUTE_UNUSED,
...
virDomainDefPtr
lxcDomainMigrationPrepareDef(virLXCDriverPtr driver ATTRIBUTE_UNUSED,
                               const char *dom_xml ATTRIBUTE_UNUSED,
                               const char *dname ATTRIBUTE_UNUSED)
{
    return NULL;
...
int
lxcDomainMigrationPrepare(virConnectPtr dconn ATTRIBUTE_UNUSED,
...
int
lxcDomainMigrationPerform(virLXCDriverPtr driver ATTRIBUTE_UNUSED,
                          virDomainObjPtr vm ATTRIBUTE_UNUSED,
                          const char *dom_xml ATTRIBUTE_UNUSED,
                          const char *dconnuri ATTRIBUTE_UNUSED,
...
virDomainPtr
lxcDomainMigrationFinish(virConnectPtr dconn ATTRIBUTE_UNUSED,
                         virDomainObjPtr vm ATTRIBUTE_UNUSED,
                         unsigned int flags_unused ATTRIBUTE_UNUSED,
...
int
lxcDomainMigrationConfirm(virLXCDriverPtr driver ATTRIBUTE_UNUSED,
                          virDomainObjPtr vm ATTRIBUTE_UNUSED,
                          unsigned int flags_unused ATTRIBUTE_UNUSED,
                          int cancelled ATTRIBUTE_UNUSED)
{
